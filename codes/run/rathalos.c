...
/* This code was developped relying heavily on E.Cipriano's work on his laminarflame.c and uses includes from his sandbox such as the Cantera wrapper.
...
// --- Kinetics ---
#define KIN_MECHANISM   "C3MechV4_RED.yaml" // Mecanism imput
#define FUEL_NAME       "IC3H7OH"           // Exact name of the fuel species in the mecanism
...
// 0 = No fuel
// 1 = Injection of premixed fuel (phi=1) from the botton
...
#define T_RESIDENCE     0.003        // Time residency of the flame
...
#define R_FLAME         5e-3         // Radius of the spherical flame (meters)

// --- Stratification Parameters (Config 3) ---
#define PHI_BOTTOM 2.0     // Bottom boundary richness
#define PHI_TOP    0.6     // Richness at Y_STRAT
...
// Various properties used for computations
#define FUEL_MW         60.096       // Isopropanol molar mass (kg/kmol)
#define O2_MW           31.998       // O2 molar mass
#define N2_MW           28.014       // N2 molar mass
#define STOICH_O2_MOLES 4.5          // Stochiometric number
#define AIR_N2_O2_RATIO 3.76         // N2/O2 ratio
...
// various computations for the domain
#define MASS_O2_STOICH  (STOICH_O2_MOLES * O2_MW)
#define MASS_N2_STOICH  (STOICH_O2_MOLES * AIR_N2_O2_RATIO * N2_MW)
#define MASS_AIR_STOICH (MASS_O2_STOICH + MASS_N2_STOICH)
#define Y_FUEL_STOICH   (FUEL_MW / (FUEL_MW + MASS_AIR_STOICH))
...
// ===================================================================
// --- 1D flame import and interpollation ---
// ===================================================================
...
void load_flame_csv(const char * filename) {
    ...
    if (pid() == 0) {
        printf("INFO : Flame profil successfully loaded (%d points).\n", flame_prof.n_points);
    }
}
...
event compute_hrr (i++) {
    ...
    foreach() {
        ...
        if (idx_FUEL >= 0) wdot_fuel[] = wdot[idx_FUEL] * FUEL_MW; 
        else wdot_fuel[] = 0.0;
    }
